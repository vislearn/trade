...
from .util import (
    get_loss_lambda_from_hparams,
    parameter_schedule,
    weight_by_temperature,
    convert_dict_to_hparams,
    get_clipped_loss_from_hparams,
)
from .losses import (
    get_correction_function,
    compute_KL,
    ModelDistribution,
    TRADE_loss_legacy,
    TRADE_loss_continuous,
    TRADE_loss_grid,
)
...
class BoltzmannGenerator(Trainable):
    ...
    def __init__(
        self,
        hparams: BoltzmannGeneratorHParams | dict,
        optuna_trial=None,
        criterion=None,
    ):
        self.parameter_name = hparams.get("target_parameter_name", "temperature")
        self.datasets = [
            get_loader(hparams["dataset"]["name"])(
                **{self.parameter_name: parameter},
                **hparams["dataset"].get("kwargs", {}),
            )
            for parameter in hparams["dataset"]["training_data_parameters"]
        ]

        hparams = convert_dict_to_hparams(
            hparams,
            dim=self.datasets[0].dim,
            dims_c=(
                None
                if not hasattr(self.datasets[0], "conditions")
                else [self.datasets[0].conditions.shape[1:]]
            ),
        )
        ...
        if self.hparams.trade_loss is not None:
            ...
            if "target_parameter_name" in additional_kwargs:
                warn(
                    f"Overwriting target_parameter_name in additional_kwargs from {additional_kwargs['target_parameter_name']} to {self.parameter_name}"
                )
            ...
            if trade_mode == "grid":
                ...
                self.trade_loss = TRADE_loss_grid(
                    param_min=min(parameters),
                ...
                self.trade_loss = TRADE_loss_continuous(
                    **additional_kwargs, condition_sampler=self.sample_condition
                )
            elif trade_mode == "legacy":
                self.trade_loss = TRADE_loss_legacy(**additional_kwargs)

        self.data_augmentor = DataAugmentor(
            hparams.data_augmentation_hparams, self.datasets[0].dim
        )
        self.parameter_prior = parameter_schedule(
            hparams.n_steps, hparams.parameter_prior_hparams
        )
        self.min_nll = 0.0
        ...
        assert [
            self.hparams.nll_loss,
            self.hparams.kl_loss,
            self.hparams.temperature_weighted_loss,
            self.hparams.trade_loss,
            self.hparams.var_loss,
        ].count(None) <= 4, "At least one loss must be specified"
    ...
    def dequantize(self, x, c):
        if not self.training:
            if self.hparams.softflow and isinstance(
                self.hparams.dequantization_noise, (list, tuple)
            ):
        ...
        if isinstance(dequantization_noise, float):
            noise_strength = dequantization_noise * torch.ones(
                x.shape[0], device=x.device
            )
            if self.hparams.softflow and self.training:
                warn(
                    "Softflow is enabled, but dequantization noise is not set to a range."
                )
        elif isinstance(dequantization_noise, (list, tuple)):
            assert (
                len(dequantization_noise) == 2
            ), "Dequantization noise must be a list or tuple of length 2"
            ...
            noise_strength = min_noise + (max_noise - min_noise) * torch.rand(
                x.shape[0], device=x.device
            )
            noise_strength = torch.exp(noise_strength)
        else:
            raise ValueError(
                f"Unknown dequantization noise format {type(dequantization_noise)}"
            )
    ...
    def compute_metrics(self, batch, batch_idx):
        ...
        if (
            self.hparams.update_min_nll_every is not None
            and current_step % self.hparams.update_min_nll_every == 0
        ):
            self.update_min_nll()

        target_parameter = (
            self.parameter_prior(current_step, x.shape[0]).unsqueeze(1).to(x.device)
        )

        self.update_adaptive_lambdas(
            x, c, data_parameter, target_parameter, current_step
        )

        lam_nll = get_loss_lambda_from_hparams(
            self.hparams.nll_loss, self.hparams.n_steps, current_step
        )
        lam_kl = get_loss_lambda_from_hparams(
            self.hparams.kl_loss, self.hparams.n_steps, current_step
        )
        lam_temp = get_loss_lambda_from_hparams(
            self.hparams.temperature_weighted_loss, self.hparams.n_steps, current_step
        )
        lam_trade = get_loss_lambda_from_hparams(
            self.hparams.trade_loss, self.hparams.n_steps, current_step
        )
        lam_var = get_loss_lambda_from_hparams(
            self.hparams.var_loss, self.hparams.n_steps, current_step
        )
        ...
        if (
            lam_var > 0 or lam_trade > 0
        ) and not self.hparams.parameter_prior_hparams.sample_parameter_per_batch:
            warn(
                "Parameter is not sampled per batch, but per sample. This can lead to unexpected behavior in the variance and trade loss, which rely on batch statistics."
            )
        ...
        if lam_nll > 0 or not self.training:
            ...
            metrics["loss"] += (
                lam_nll
                * get_clipped_loss_from_hparams(self.hparams.nll_loss, nll).mean()
            )
        ...
        if lam_kl > 0 or (not self.training and self.datasets[0].has_energy):
            ...
            metrics["kl"] = kl.mean()
            metrics["loss"] += (
                lam_kl * get_clipped_loss_from_hparams(self.hparams.kl_loss, kl).mean()
            )

        if lam_var > 0 or (not self.training and self.datasets[0].has_energy):
            # check if kl already has been computed
            try:
                proposal_distribution = self.hparams.var_loss.additional_kwargs.get(
                    "proposal", "model"
                )
            ...
            if proposal_distribution == "model" and "kl" in metrics:
                ...
                var_loss = self.get_variance_loss(
                    x.shape[0],
                    self.sample_condition(x.shape[0]),
                    target_parameter,
            ...
            metrics["loss"] += (
                lam_var
                * get_clipped_loss_from_hparams(self.hparams.var_loss, var_loss).mean()
            )
        ...
        if lam_temp > 0 or (not self.training and self.parameter_name == "temperature"):
            if "nll" in metrics:
                ...
                nll_base = None
            temp_weighted_loss = self.get_temperature_weighted_loss(
                x, c, data_parameter, target_parameter, nll_base=nll_base
            )
            metrics["temperature_weighted_loss"] = (temp_weighted_loss).mean()
            metrics["loss"] += (
                lam_temp
                * get_clipped_loss_from_hparams(
                    self.hparams.temperature_weighted_loss, temp_weighted_loss
                ).mean()
            )
        ...
        if (lam_trade > 0 or not self.training) and hasattr(self, "trade_loss"):
            ...
            metrics["loss"] += (
                lam_trade
                * get_clipped_loss_from_hparams(
                    self.hparams.trade_loss, trade_loss
                ).mean()
            )
    ...
    def get_temperature_weighted_loss(
        self, x, c, data_parameter, target_parameter, nll_base=None
    ):
        assert (
            self.parameter_name == "temperature"
        ), "Temperature weighted loss only works with temperature as parameter"
        try:
            self_supervised = (
                self.hparams.temperature_weighted_loss.additional_kwargs.get(
                    "self_supervised", True
                )
            )
            reference_temperature = (
                self.hparams.temperature_weighted_loss.additional_kwargs.get(
                    "relative_temperature", "training"
                )
            )
        except AttributeError:
            self_supervised = True
        ...
        if self_supervised:
            if reference_temperature == "training":
                ...
                new_parameter = torch.rand_like(target_parameter) * torch.abs(
                    target_parameter - 1.0
                ) + torch.where(target_parameter > 1.0, 1.0, target_parameter)
                ...
                new_parameter = target_parameter + torch.where(
                    target_parameter > 1.0,
                    torch.clip(1.0 - target_parameter, -0.1, 0.0),
                    torch.clip(1.0 - target_parameter, 0.0, 0.1),
                )
    ...
    def get_trade_loss(self, x, c, data_parameter, target_parameter):
        if isinstance(self.trade_loss, TRADE_loss_legacy):
            ...
            reference_parameter = self.hparams.trade_loss.additional_kwargs.get(
                "reference_parameter"
            )
            if not self.hparams.trade_loss.additional_kwargs.get(
                "self_supervised", True
            ):
                reference_parameter = data_parameter
            return self.trade_loss(
                flow=self.flow,
            ...
            return self.trade_loss(
                flow=self.flow,
                base_parameter=self.hparams.trade_loss.additional_kwargs.get(
                    "reference_parameter"
                ),
    ...
    def get_variance_loss(self, batch_size, c, target_parameter, proposal="model"):
        if proposal == "model":
            ...
            return self.flow.energy_ratio_from_latent(
                z, c, parameter=target_parameter
            ).var()
    ...
    def get_grad_magnitude(self, loss_i):
        ...
        num_params = sum(
            [
                param.numel()
                for _, param in self.flow.named_parameters()
    ...
    def update_adaptive_lambdas(
        self, x, c, data_parameter, target_parameter, current_step
    ):
        ...
        if (
            current_step % self.hparams.update_adaptive_weights_every != 0
            or not any(
                [
                    config.adaptive
                    and config.pct_start <= current_step / self.hparams.n_steps
        ...
        losses = [
            self.get_nll_loss,
            self.get_kl_loss,
            self.get_temperature_weighted_loss,
            self.get_trade_loss,
            self.get_variance_loss,
        ]

        grad_mags = np.zeros(len(configs))
        adaptive_configs = []

        for i, (config, loss, loss_input) in enumerate(
            zip(configs, losses, loss_inputs)
        ):
            if (
                config is not None
                and config.pct_start <= current_step / self.hparams.n_steps
                ...
                if config.adaptive:
                    ...
                    warn(
                        f"Some loss weights are adaptive, while others are not. This can lead to problems in normalization."
                    )
                    lam_loss_i = get_loss_lambda_from_hparams(
                        config, self.hparams.n_steps, current_step
                    )
        ...
        for i, (_, config) in enumerate(adaptive_configs):
            config.adaptive_weight = (
                config.alpha_adaptive_update * grad_mags[i]
                + (1 - config.alpha_adaptive_update) * config.adaptive_weight
            )
    ...
    def sanity_check(self):
        ...
        with torch.no_grad():
            assert torch.all(
                (batch - self.flow(self.flow(batch)[0], rev=True)[0]) ** 2 < 1e-5
            ), "Sanity check failed"
    ...
    def on_train_epoch_end(self):
        if self.current_epoch % self.hparams.plotting[
            "interval"
        ] == 0 or self.current_epoch == (self.hparams.max_epochs - 1):
            try:
                self.plot()
            except Exception as e:
                # No need to stop a run because of a plotting error
                print(
                    f"Plotting failed in epoch {self.current_epoch} with error {traceback.format_exc()}"
                )
    ...
    def sample_condition(self, batch_size, train=True):
        if train:
            ...
            _, c = self.dequantize(
                torch.zeros(batch_size, self.datasets[0].dim, device=self.device), c
            )
        else:
    ...
    def plot(self):
        ...
        if self.parameter_name == "temperature":
            T_0 = 1.0
            p_T_0 = ModelDistribution(
                self.flow,
                self.sample_condition,
                parameter=T_0,
                n_repeats=self.hparams.plotting.get("n_repeats_log_prob", 1),
            )
            q = ModelDistribution(
                self.flow,
                self.sample_condition,
            ...
            correction = get_correction_function(
                p_T_0,
                q,
                n_proposal_samples=self.hparams.plotting.get(
                    "n_proposal_samples", 100000
                ),
            )
            ...
            for parameter in self.hparams.plotting["parameters"]:
                ...
                kl_divs.append(
                    compute_KL(
                        ...
                        T_star,
                        T_0,
                        n_KL_samples=self.hparams.plotting.get("n_KL_samples", 10000),
                    )
                )
            kl_divs = torch.stack(kl_divs, dim=0).cpu().numpy()
            plot_consistency_check(
                kl_divs,
                self.hparams.plotting["parameters"],
                self.logger.experiment,
                self.current_epoch,
            )
